...
const { NumberConvert, UpdateMoneyWallet, Format, getUser, isStaff } = require('../../utils/functions.js');

module.exports = {
  name: "addcarteira",
  aliases: ["adicionarcarteira", "darcarteira", "darmoedas"],
  description: "Adiciona uma quantia na carteira de um usuário.",
  ...
  run: async (client, message, args, prefixo, color, database, emoji) => {
    try {
      ...
      if (!args[0]) {
        return message.reply({
          content: `📖 **Uso Correto:**\n> \`${prefixo}addcarteira <quantia> [@usuário/ID]\` ou \`${prefixo}addcarteira <@usuário/ID> <quantia>\`\n\n💡 **Exemplo:** \`${prefixo}addcarteira 50000 @user\` ou \`${prefixo}addcarteira 10k\``
        });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle('💵 Saldo na Carteira Adicionado')
        .setDescription(
          `${emoji.positivo || '✅'} **|** Foram adicionados **${Format(quantiaRaw)}** na carteira de <@${targetUser.id}>!\n\n` +
      ...
      console.error('[Command addcarteira]', error);
      return message.reply({
        content: `${emoji.negativo || '❌'} **|** Ocorreu um erro ao adicionar dinheiro na carteira.`
      });
    }
  }
};
