...
const { NumberConvert, UpdateMoneyWallet, Format, getUser, isStaff, getUserMoney } = require('../../utils/functions.js');

module.exports = {
  name: "removecarteira",
  aliases: ["removercarteira", "delcarteira", "tirarmoedas"],
  description: "Remove uma quantia da carteira de um usuário.",
  ...
  run: async (client, message, args, prefixo, color, database, emoji) => {
    try {
      ...
      if (!args[0]) {
        return message.reply({
          content: `📖 **Uso Correto:**\n> \`${prefixo}removecarteira <quantia> [@usuário/ID]\` ou \`${prefixo}removecarteira <@usuário/ID> <quantia>\`\n\n💡 **Exemplo:** \`${prefixo}removecarteira 50000 @user\` ou \`${prefixo}removecarteira 10k\``
        });
      }
      ...
      const atualCarteira = userMoney.carteira || 0;
      const quantiaRemover = Math.min(atualCarteira, quantiaRaw);

      if (quantiaRemover <= 0) {
        return message.reply({
          content: `${emoji.aviso || '⚠️'} **|** O usuário <@${targetUser.id}> já possui carteira zerada.`
        });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle('💵 Saldo na Carteira Removido')
        .setDescription(
          `${emoji.negativo || '❌'} **|** Foram removidos **${Format(quantiaRemover)}** da carteira de <@${targetUser.id}>!\n\n` +
          `👤 **Usuário afetado:** \`${targetUser.username || targetUser.tag || targetUser.id}\` (\`${targetUser.id}\`)\n` +
          `💰 **Saldo restante na carteira:** \`${Format(atualCarteira - quantiaRemover)}\`\n` +
      ...
      console.error('[Command removecarteira]', error);
      return message.reply({
        content: `${emoji.negativo || '❌'} **|** Ocorreu um erro ao remover dinheiro da carteira.`
      });
    }
  }
};
